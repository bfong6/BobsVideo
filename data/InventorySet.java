...
final class InventorySet implements Inventory {
	...
	Record checkOut(Video video) {
		if (_data.containsKey(video)) {
			RecordObj r = (RecordObj) _data.get(video);
			RecordObj copy = r.copy();
			if (r.numOut == r.numOwned) throw new IllegalArgumentException();
			r.numOut++;
			r.numRentals++;
			return copy;
	...
	Record checkIn(Video video) {
		if (_data.containsKey(video)) {
			RecordObj r = (RecordObj) _data.get(video);
			RecordObj copy = r.copy();
			if (r.numOut == 0) throw new IllegalArgumentException();
			r.numOut--;
			return copy;
		} else throw new IllegalArgumentException();
	}
	 ...
	 */
	Map clear() {
		InventorySet copy = new InventorySet();
		for (Record r : _data.values()) {
			Video v = r.video();
			copy.replaceEntry(v, r);
		}
		_data.clear();
		return copy._data;
	}
